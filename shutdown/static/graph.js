/* Evidence graph — canvas force layout.
   Hand-written rather than pulling in d3: the simulation is three forces and
   ~120 lines, keeping the app dependency-free and offline, same reasoning as
   galaxy.js/signalfield.js.
   ...
   Colors are the app's actual dark-theme tokens (see styles.css :root),
   restated as hex here since canvas can't read CSS custom properties. */
const OK = "#34d399", WARN = "#f2b84b", BAD = "#ff6b57", CONTROL = "#7c6cff";
const REL = {
  supports:    { color: OK,   label: "supports" },
  weakens:     { color: WARN, label: "weakens" },
  refutes:     { color: BAD,  label: "refutes" },
  unknown:     { color: "#8a7fa0", label: "unknown" },
  verified_by: { color: CONTROL, label: "verified by" },
};
const STATUS = { survived: OK, eliminated: BAD, alive: WARN };
...
export class EvidenceGraph {
  ...
  resize() {
    ...
    const w = (p ? p.clientWidth : 800), h = (p ? p.clientHeight : 420);
    this.w = w; this.h = h;
  ...
  draw() {
    ...
    for (const n of this.nodes) {
      ...
      if (n.kind === "hypothesis") {
        ...
        g.fillStyle = "#0d0a16"; g.beginPath(); g.arc(0, 0, r - 4, 0, 7); g.fill();
        ...
        g.rotate(Math.PI / 4);
        g.fillStyle = CONTROL + "22"; g.strokeStyle = CONTROL; g.lineWidth = 1.4;
        g.fillRect(-r, -r, r * 2, r * 2); g.strokeRect(-r, -r, r * 2, r * 2);
      } else {
        g.beginPath(); g.arc(0, 0, r, 0, 7);
        g.fillStyle = "#1a1526"; g.fill();
        g.strokeStyle = "#362d47"; g.lineWidth = 1.3; g.stroke();
        g.beginPath(); g.arc(0, 0, 3, 0, 7); g.fillStyle = "#8a7fa0"; g.fill();
      }
      g.restore();
    }
  ...
  tooltip(n) {
    ...
    g.save();
    g.fillStyle = "rgba(8,6,13,.96)"; g.strokeStyle = "#362d47"; g.lineWidth = 1;
    g.beginPath(); g.roundRect(x, y, w, h, 9); g.fill(); g.stroke();
    g.textAlign = "left"; g.textBaseline = "top";
    wrap.forEach((l, i) => {
      g.fillStyle = i === 0 ? "#f4f0fb" : "#9d94b8";
